...
public class RemoteObjectServerImpl implements RemoteObjectServer {
	...
	private static class RemoteReferenceInternalData<T> implements TimeoutAble {
		...
		public boolean equals(final Object obj) {
			...
			return eachEquality(of(this.remoteReference, this.lastDateAccess),
					andOf(that.remoteReference, that.lastDateAccess));
		}
	...
	private static class UserTokenInternalData implements TimeoutAble {
		 ...
		 *            the user token
		 */
		public UserTokenInternalData(final UserToken userToken) {
			super();
		...
		public boolean equals(final Object obj) {
			...
			return eachEquality(of(userToken, lastDateAccess), andOf(
					that.userToken, that.lastDateAccess));
		}
	...
	public synchronized static RemoteObjectServer getDefault(
			final UserAuthenticator userAuthenticator, final Integer portToUse,
			final Long timeoutInMilliseconds) {
		if (defaultReference == null) {
			defaultReference = new RemoteObjectServerImpl(userAuthenticator,
					portToUse, timeoutInMilliseconds);
		} else {
			checkCondition("sameUserAutenticator", userAuthenticator
					.equals(defaultReference.userAuthenticator));
			checkCondition("samePort", portToUse
					.equals(defaultReference.portToUse));
			checkCondition("sameTimeoutInMilliseconds", timeoutInMilliseconds
					.equals(defaultReference.timeoutInMilliseconds));
		}
	 ...
	 *            the timeout in milliseconds
	 */
	private RemoteObjectServerImpl(final UserAuthenticator userAuthenticator,
		...
		try {
			...
			checkCondition("timeoutInMinutesBiggerThanOrEqualToZero",
					timeoutInMilliseconds.longValue() >= 0L);
			...
			throw logAndReturnNew(format(
					"Problem starting remote object server inside port {0}",
					portToUse), e, ConfigurationException.class);
		}
	}

	public synchronized void closeAllObjects() {
		for (final Entry<RemoteReference<?>, RemoteReferenceInternalData<?>> e : remoteReferences
				.entrySet()) {
	...
	public <T> RemoteReference<T> createRemoteReference(
			final UserToken userToken, final Class<T> remoteReferenceType,
			final Object... parameters) throws InvalidReferenceTypeException {
		try {
			checkNotNull("userToken", userToken);

			checkNotNull("remoteReferenceType", remoteReferenceType);
			checkCondition("remoteReferenceTypeIsInterface",
					remoteReferenceType.isInterface());
			checkCondition("remoteReferenceTypeContainsFactory",
					internalObjectFactoryMap.containsKey(remoteReferenceType));
			checkCondition("isUserTokenValid", isUserTokenValid(userToken));

			final InternalObjectFactory<T> internalFactory = (InternalObjectFactory<T>) internalObjectFactoryMap
					.get(remoteReferenceType);

			final T newObject = internalFactory.createNewInstance(parameters);

			final RemoteReference<T> reference = this
					.internalCreateRemoteReference(userToken,
							remoteReferenceType, newObject);
	 ...
	 */
	public UserToken createUserToken(final String user, final String password,
		...
		final boolean canConnect = userAuthenticator.canConnect(user, password,
				clientHost);
		if (!canConnect) {
			throw logAndReturn(new AccessDeniedException(
					format(
							"User {0} from host {1} can't connect to this server instance",
							user, clientHost)));
		}

		final UserToken token = new UserToken(user, UUID.randomUUID()
				.toString());
	...
	private void garbageCollection() {
		while (true) {
			...
			if (logger.isTraceEnabled()) {
				logger
						.trace(format(
								...
								acceptableDiff, timeoutInMilliseconds,
								curTimeMillis));
			}

			final Set<UserTokenInternalData> deathUserEntries = new HashSet<UserTokenInternalData>();
			final Set<Entry<UserToken, UserTokenInternalData>> userEntries = userTokenDataMap
					.entrySet();
			...
			for (final UserTokenInternalData deathUserEntry : deathUserEntries) {
				logger.info(format("removing user {0} token {1}",
						deathUserEntry.getUserToken().getUser(), deathUserEntry
								.getUserToken().getToken()));

				userTokenDataMap.remove(deathUserEntry.getUserToken());
				final Set<Entry<RemoteReference<?>, RemoteReferenceInternalData<?>>> remoteObjectsEntries = remoteReferences
						.entrySet();
	 ...
	 */
	@SuppressWarnings("unchecked")
	private <T> RemoteReference<T> internalCreateRemoteReference(
			final UserToken userToken, final Class<T> remoteReferenceType,
			final T newObject) {
		if (newObject == null) {
			return null;
		...
		if (newObject != null) {
			for (final Entry<RemoteReference<?>, RemoteReferenceInternalData<?>> entry : remoteReferences
					.entrySet()) {
		...
		reference = new RemoteReference<T>(remoteReferenceType, newObject
				.getClass().getInterfaces(), remoteReferenceId, userToken);
		remoteReferences.put(reference, new RemoteReferenceInternalData<T>(
				reference, newObject));
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T, R> AbstractInvocationResponse<R> invokeRemoteMethod(
			final RemoteObjectInvocation<T> invocation)
		...
		try {
			final RemoteReferenceInternalData<T> remoteReferenceData = (RemoteReferenceInternalData<T>) remoteReferences
					.get(invocation.getRemoteReference());
			final T object = remoteReferenceData.getObject();
			Method method = null;

			for (final Class<?> iface : invocation.getRemoteReference()
					.getInterfaces()) {
				if (iface.equals(ObjectMethods.class)) {
					continue;
				}
				try {
					method = iface.getMethod(invocation.getMethodName(),
			...
			if (method == null) {
				try {
					method = Object.class.getMethod(invocation.getMethodName(),
							invocation.getParameterTypes());
			...
			checkCondition("methodNotNull:" + invocation.getMethodName(),
					method != null);
			...
			for (int i = 0, size = args.length; i < size; i++) {
				if (args[i] instanceof RemoteReference<?>) {
					...
					args[i] = remoteReferences.get(ref).getObject();
					...
					if (o instanceof RemoteReference<?>) {
						...
						for (final Object item : collection) {
							if (item instanceof RemoteReference<?>) {
								...
								final Object correctInstance = remoteReferences
										.get(remoteRef).getObject();
					...
					final Iterator<Entry<Object, Object>> it = map.entrySet()
							.iterator();
					Object o = null;
					while (o == null) {
						o = it.next().getValue();
					...
					if (o instanceof RemoteReference<?>) {
						...
						for (final Entry<Object, Object> entry : map.entrySet()) {
							if (entry.getValue() instanceof RemoteReference<?>) {
								final RemoteReference<Object> remoteRef = (RemoteReference<Object>) entry
										.getValue();
								final Object correctInstance = remoteReferences
										.get(remoteRef).getObject();
			...
			final R result = (R) method.invoke(object, invocation
					.getParameters());
			...
			if (isRemote(method)) {
				if (result instanceof Collection) {
					...
					final AbstractInvocationResponse<R> response = (AbstractInvocationResponse<R>) this
							.wrapResultIntoCollection(method, invocation
									.getUserToken(), (Collection<?>) result);
					return response;
				}
				if (result instanceof Map) {
					final AbstractInvocationResponse<R> response = (AbstractInvocationResponse<R>) this
							.wrapResultIntoMap(method, invocation
									.getUserToken(), (Map<?, ?>) result);

					return response;
				}
				final RemoteReference<R> remoteReference = this
						.internalCreateRemoteReference(invocation
								.getUserToken(), (Class<R>) invocation
			...
			throw logAndReturnNew(e,
					InternalErrorOnMethodInvocationException.class);
		} finally {
	...
	private boolean isRemote(final Method method) throws Exception {
		...
		if (Collection.class.isAssignableFrom(returnType)) {
			final UnwrappedCollectionTypeFromMethodReturn<Object> metadata = Reflection
					.unwrapCollectionFromMethodReturn(method);
			...
			final UnwrappedMapTypeFromMethodReturn<Object, Object> metadata = Reflection
					.unwrapMapFromMethodReturn(method);
	 ...
	 */
	private boolean isRemoteReferenceValid(
			final RemoteReference<?> remoteReference)
			throws RemoteReferenceInvalid, UserTokenInvalid {
		checkNotNull("remoteReference", remoteReference);
		checkCondition("userTokenValid", isUserTokenValid(remoteReference
				.getUserToken()));
		final boolean contains = remoteReferences.containsKey(remoteReference);
		if (!contains) {
			throw logAndReturn(new RemoteReferenceInvalid(remoteReference
					.getRemoteType()
	...
	private boolean isUserTokenValid(final UserToken userToken)
		...
		if (!contains) {
			throw logAndReturn(new UserTokenInvalid(userToken.getUser()
					+ " is invalid. Try to connect again."));
		}
		return true;
	}
	 ...
	 *            the death entry
	 */
	private void removeDeathEntry(
			final RemoteReferenceInternalData<?> deathEntry) {
		try {
			logger.info(format("removing reference {0} id {1}", deathEntry
					.getObject().toString(), deathEntry.getRemoteReference()
					.getRemoteReferenceId()));

		} catch (final Exception e) {
			logger
					.warn("error printing log for death entry "
							+ deathEntry.getRemoteReference()
									.getRemoteReferenceId(), e);
		}
		remoteReferences.remove(deathEntry.getRemoteReference());
		final Method[] methods = deathEntry.getObject().getClass().getMethods();
		for (final Method m : methods) {
			if (m.isAnnotationPresent(DisposeMethod.class)
					&& m.getParameterTypes().length == 0) {
				final DisposeMethod disposeAnnotation = m
						.getAnnotation(DisposeMethod.class);
	...
	public synchronized void shutdown() {
		...
		closed.set(true);
		for (final Entry<Class<?>, InternalObjectFactory<?>> entry : internalObjectFactoryMap
				.entrySet()) {
	 ...
	 *            the remote reference
	 */
	private void updateRemoteReferenceIfNecessary(
			final RemoteReference<?> remoteReference) {
		checkNotNull("remoteReference", remoteReference);
		if (remoteReferences.containsKey(remoteReference)) {
			remoteReferences.get(remoteReference).getLastDateAccess().set(
	...
	private void updateUserToken(final UserToken userToken) {
		checkNotNull("userToken", userToken);
		checkCondition("userTokenValid", userTokenDataMap
				.containsKey(userToken));
		userTokenDataMap.get(userToken).getLastDateAccess().set(
				System.currentTimeMillis());
	}

	@SuppressWarnings("unchecked")
	public <W, R extends Collection<W>> AbstractInvocationResponse<R> wrapResultIntoCollection(
			final Method method, final UserToken userToken, final R collection)
			throws Exception {
		final UnwrappedCollectionTypeFromMethodReturn<Object> metadata = Reflection
				.unwrapCollectionFromMethodReturn(method);
		...
		for (final W o : collection) {
			final RemoteReference<W> remoteRef = this
					.internalCreateRemoteReference(userToken,
							remoteReferenceType, o);
		...
		final CollectionOfRemoteInvocationResponse<W, R> wrapped = new CollectionOfRemoteInvocationResponse<W, R>(
				(Class<R>) collectionType, remoteReferencesCollection);
		return wrapped;
	}

	@SuppressWarnings("unchecked")
	public <K, W, R extends Map<K, W>> AbstractInvocationResponse<R> wrapResultIntoMap(
			final Method method, final UserToken userToken, final R map)
			throws Exception {
		final UnwrappedMapTypeFromMethodReturn<Object, Object> metadata = Reflection
				.unwrapMapFromMethodReturn(method);
		final Class<W> remoteReferenceType = (Class<W>) metadata.getItemType()
				.getK2();
		final Map<K, RemoteReference<W>> remoteReferencesMap = new HashMap<K, RemoteReference<W>>(
				map.size());
		for (final Entry<K, W> o : map.entrySet()) {
			final RemoteReference<W> remoteRef = this
					.internalCreateRemoteReference(userToken,
							remoteReferenceType, o.getValue());
			remoteReferencesMap.put(o.getKey(), remoteRef);
		}

		final MapOfRemoteInvocationResponse<K, W, R> wrapped = new MapOfRemoteInvocationResponse<K, W, R>(
				remoteReferencesMap);
		return wrapped;
	}
}
