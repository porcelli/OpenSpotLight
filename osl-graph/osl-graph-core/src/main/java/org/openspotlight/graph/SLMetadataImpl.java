...
public class SLMetadataImpl implements SLMetadata {
    ...
    public Collection<SLMetaNodeType> searchMetaNodeType( final SLRecursiveMode recursiveMode,
        ...
        synchronized (lock) {
            try {
                final String statement = SLMetadataXPathSupporter.buildXpathForMetaNodeType(treeSession
                                                                                                       .getXPathRootPath()
                                                                                            + "/metadata/types", recursiveMode, visibility, property2Find, logicOp, booleanOp, values);
