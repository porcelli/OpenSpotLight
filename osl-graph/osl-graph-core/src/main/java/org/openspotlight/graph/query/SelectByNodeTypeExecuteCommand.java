...
public class SelectByNodeTypeExecuteCommand extends SelectAbstractCommand {
	 ...
	 *            the command do
	 */
	SelectByNodeTypeExecuteCommand(SelectByNodeTypeInfo selectInfo,
	...
	private Map<String, SLWhereTypeInfo> getWhereTypeInfoMap()
		...
		List<SLWhereTypeInfo> list = selectInfo.getWhereStatementInfo()
	 ...
	 */
	private void filterByWhereStatement(Statement statement, String typeName,
	...
	private Set<SLSelectTypeInfo> getSelectTypeInfoSet() {
		...
		if (selectInfo.getAllTypes() != null) {
			if (selectInfo.getAllTypes().isOnWhere()) {
				List<SLWhereTypeInfo> whereTypeInfoList = selectInfo
						.getWhereStatementInfo().getWhereTypeInfoList();
				for (SLWhereTypeInfo whereTypeInfo : whereTypeInfoList) {
					SLSelectTypeInfo typeInfo = new SLSelectTypeInfo(
							whereTypeInfo.getName());
					typeInfo.setSubTypes(whereTypeInfo.isSubTypes());
					set.add(typeInfo);
				}
			} else {
				Iterable<MetaNodeType> metaNodeTypes = metadata
						.getMetaNodesTypes();
				for (MetaNodeType metaNodeType : metaNodeTypes) {
					SLSelectTypeInfo typeInfo = new SLSelectTypeInfo(
