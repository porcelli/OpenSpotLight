...
public class SLQueryApiImpl extends AbstractSLQuery implements SLQueryApi {
    ...
    private Collection<SLMetaNodeType> getMetaNodeTypes( final String name,
        ...
        if (name.equals(SLNode.class.getName())) {
            ;
            for (SLMetaNodeType metaNodeType : this.metadata.getMetaNodesTypes()) {
    ...
    private void validateTypesExsistence( final SLSelectStatementInfo selectInfo )
        ...
        if (whereInfo != null) {
             ...
             * (!linkTypeExists(linkTypeInfo.getName())) { throw new SLInvalidQueryElementException
             * ("Link type on where clause not found: " + linkTypeInfo.getName()); } }
             **/
        }
    }
