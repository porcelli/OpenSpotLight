...
class PLinkNodeWrapper {
    ...
    public Integer getLinkTypeHash() throws SLPersistentTreeSessionException {
        if (this.linkTypeHash == null) {
            this.linkTypeHash = SLCommonSupport.getInternalPropertyAsInteger(this.pLinkNode,
                                                                             SLConsts.PROPERTY_NAME_LINK_TYPE_HASH);
        }
    ...
    public String getSourceID() throws SLPersistentTreeSessionException {
        if (this.sourceID == null) {
            this.sourceID = SLCommonSupport.getInternalPropertyAsString(this.pLinkNode, SLConsts.PROPERTY_NAME_SOURCE_ID);
        }
    ...
    public String getTargetID() throws SLPersistentTreeSessionException {
        if (this.targetID == null) {
            this.targetID = SLCommonSupport.getInternalPropertyAsString(this.pLinkNode, SLConsts.PROPERTY_NAME_TARGET_ID);
        }
...
class PNodeWrapper {
    ...
    public String getName() throws SLPersistentTreeSessionException {
        if (this.name == null) {
            this.name = SLCommonSupport.getInternalPropertyAsString(this.pNode, SLConsts.PROPERTY_NAME_DECODED_NAME);
        }
        return this.name;
    }
    ...
    public String getParentName() throws SLPersistentTreeSessionException {
        if (this.parentName == null) {
            ...
            this.parentName = SLCommonSupport.getInternalPropertyAsString(pParentNode, SLConsts.PROPERTY_NAME_DECODED_NAME);
        }
    ...
    public Comparable<Serializable> getPropertyValue( final String name ) throws SLPersistentTreeSessionException {
        Comparable<Serializable> comparableValue = this.propertyValueMap.get(name);
        if (comparableValue == null) {
            final Serializable value = SLCommonSupport.getUserPropertyAsSerializable(this.pNode, name);
    ...
    public String getTypeName() throws SLPersistentTreeSessionException {
        if (this.typeName == null) {
            this.typeName = SLCommonSupport.getInternalPropertyAsString(this.pNode, SLConsts.PROPERTY_NAME_TYPE);
        }
    ...
    public String toString() {
        try {
            return SLCommonSupport.getInternalPropertyAsString(this.pNode, SLConsts.PROPERTY_NAME_DECODED_NAME);
        } catch (final SLPersistentTreeSessionException e) {
...
public class SLQueryApiImpl extends AbstractSLQuery implements SLQueryApi {
    ...
    public SLQueryResult execute( final String[] inputNodesIDs,
        ...
        try {
            ...
            final String queryId = this.cache.buildQueryId(this.selects, this.collatorStrength, inputNodesIDs, sortMode, limit,
                                                           offset);
            ...
            final Set<PNodeWrapper> wrappers = SLQuerySupport.getNodeWrappers(this.treeSession, inputNodesIDs);
            commandDO.setPreviousNodeWrappers(wrappers);

            final SLSelectStatementInfo lastSelectInfo = this.getLastSelect();
            resultNodeWrappers = this.getResultCollection(lastSelectInfo, sortMode);
            ...
            for (final SLSelect select : this.selects) {
                ...
                final SLSelectAbstractCommand command = SLSelectAbstractCommand.getCommand(select, selectStatementInfo, commandDO);
                commandDO.setCollatorStrength(this.getCollatorStrength(selectStatementInfo));
                resultSelectNodeWrappers = this.getResultCollection(selectStatementInfo, sortMode);
                ...
                resultSelectNodeWrappers = this.applyLimitOffset(resultSelectNodeWrappers, selectStatementInfo.getLimit(),
            ...
            resultNodeWrappers = this.applyLimitOffset(resultNodeWrappers, limit, offset);
    ...
    private SLSelectStatementInfo getLastSelect() {
        if (!this.selects.isEmpty()) {
            return SLQuerySupport.getSelectStatementInfo(this.selects.get(this.selects.size() - 1));
        }
        return null;
    }
    ...
    private Collection<SLMetaNodeType> getMetaNodeTypes( final String name,
        ...
        SLMetaNodeType metaNodeType = this.metadata.findMetaNodeTypeByDescription(name);
    ...
    private Comparator<PNodeWrapper> getOrderByPNodeWrapperComparator( final SLOrderByStatementInfo orderByStatementInfo ) {
        return new Comparator<PNodeWrapper>() {
                ...
                try {
                    ...
                    if (index1 == index2) {
                        if (nodeWrapper1.getID().equals(nodeWrapper2.getID())) {
                            ...
                            return this.normalizeCompareValue(compareValue, typeInfo.getOrderType());
                        }
                    } else {
            ...
            private int getTypeIndex( final String typeName ) throws SLGraphSessionException {
                final List<SLOrderByTypeInfo> typeInfoList = orderByStatementInfo.getOrderByTypeInfoList();
            ...
            private boolean isInstanceOf( final String subTypeName,
                                          final SLMetaNodeType metaNodeType ) throws SLGraphSessionException {
            ...
            private boolean isInstanceOf( final String subTypeName,
                ...
                if (!status) {
                    final SLMetaNodeType metaNodeType = SLQueryApiImpl.this.metadata.findMetaNodeType(typeName);
    ...
    private Collection<PNodeWrapper> getResultCollection( final SLSelectStatementInfo selectStatementInfo,
        ...
        if (orderByStatementInfo == null) {
            if (sortMode.equals(SortMode.SORTED)) {
                final Comparator<PNodeWrapper> comparator = this.getPNodeWrapperComparator();
                return new TreeSet<PNodeWrapper>(comparator);
            } else {
                return new HashSet<PNodeWrapper>();
            }
        } else {
            final Comparator<PNodeWrapper> comparator = this.getOrderByPNodeWrapperComparator(orderByStatementInfo);
            return new TreeSet<PNodeWrapper>(comparator);
        }
    }
    ...
    boolean linkTypeExists( final String name ) throws SLGraphSessionException {
        return this.metadata.getMetaLinkType(name) != null || this.metadata.getMetaLinkTypeByDescription(name) != null;
    }
     ...
     * @throws SLGraphSessionException the SL graph session exception
     */
    private boolean nodeTypeExists( final String name ) throws SLGraphSessionException {
        return this.metadata.findMetaNodeType(name) != null || this.metadata.findMetaNodeTypeByDescription(name) != null;
    }
     ...
     * @throws SLGraphSessionException the SL graph session exception
     */
    private void normalizeSelectStatementInfo( final SLSelectStatementInfo selectInfo ) throws SLGraphSessionException {
        ...
        if (selectInfo.getAllTypes() != null) {
            if (selectInfo.getAllTypes().isOnWhere()) {
                ...
                for (final SLWhereTypeInfo whereTypeInfo : whereTypeInfoList) {
                    final Collection<SLMetaNodeType> metaNodeTypes = this.getMetaNodeTypes(whereTypeInfo.getName(),
                ...
                final Collection<SLMetaNodeType> metaNodeTypes = this.metadata.getMetaNodesTypes(SLRecursiveMode.RECURSIVE);
                for (final SLMetaNodeType metaNodeType : metaNodeTypes) {
                    final SLSelectTypeInfo selectTypeInfo = new SLSelectTypeInfo(selectInfo, metaNodeType.getTypeName());
                    selectTypeInfoSet.add(selectTypeInfo);
                }
            ...
            for (final SLSelectTypeInfo selectTypeInfo : selectInfo.getTypeInfoList()) {
                final Collection<SLMetaNodeType> metaNodeTypes = this.getMetaNodeTypes(selectTypeInfo.getName(),
    ...
    private void validateLinkTypeStatementInfo( final SLLinkTypeStatementInfo linkTypeStatementInfo )
        ...
        for (final SLLinkTypeConditionInfo conditionInfo : conditionInfoList) {
            if (conditionInfo.getInnerStatementInfo() != null) {
                this.validateLinkTypeStatementInfo(conditionInfo.getInnerStatementInfo());
            }
        }
    }
    ...
    private void validateSelects() throws SLInvalidQuerySyntaxException, SLInvalidQueryElementException, SLQueryException {
        try {
            for (final SLSelect select : this.selects) {
                final SLSelectStatementInfo selectStatementInfo = SLQuerySupport.getSelectStatementInfo(select);
                this.validateSelectStatementInfoBeforeNormalization(selectStatementInfo);
                this.normalizeSelectStatementInfo(selectStatementInfo);
                this.validateSelectStatementInfoAfterNormalization(selectStatementInfo);
            }
    ...
    private void validateTypesExsistence( final SLSelectStatementInfo selectInfo )
        ...
        /**
         * for (SLSelectByLinkInfo byLinkInfo : selectInfo.getByLinkInfoList()) { if (!linkTypeExists(byLinkInfo.getName())) {
        ...
        if (whereInfo != null) {
            ...
            /**
             * for (SLWhereLinkTypeInfo linkTypeInfo : whereInfo.getWhereLinkTypeInfoList()) { if
             * (!linkTypeExists(linkTypeInfo.getName())) { throw new
             * SLInvalidQueryElementException("Link type on where clause not found: " + linkTypeInfo.getName()); } }
             **/
        }
    }
    ...
    private void validateTypeStatementInfo( final SLTypeStatementInfo typeStatementInfo ) throws SLInvalidQuerySyntaxException {
        ...
        for (final SLTypeConditionInfo conditionInfo : conditionInfoList) {
            if (conditionInfo.getInnerStatementInfo() != null) {
                this.validateTypeStatementInfo(conditionInfo.getInnerStatementInfo());
            }
        }
    }
    ...
    private void validateWhereStatements( final SLSelectStatementInfo selectInfo ) throws SLInvalidQuerySyntaxException {
        ...
        if (whereStatementInfo != null) {
            for (final SLWhereTypeInfo whereTypeInfo : whereStatementInfo.getWhereTypeInfoList()) {
                this.validateTypeStatementInfo(whereTypeInfo.getTypeStatementInfo());
            }
            for (final SLWhereLinkTypeInfo whereTypeInfo : whereStatementInfo.getWhereLinkTypeInfoList()) {
                this.validateLinkTypeStatementInfo(whereTypeInfo.getLinkTypeStatementInfo());
            }
        }
    }
}
