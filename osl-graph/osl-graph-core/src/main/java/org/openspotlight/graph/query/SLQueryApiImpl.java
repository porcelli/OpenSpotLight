...
public class SLQueryApiImpl extends AbstractSLQuery implements SLQueryApi {
    ...
    public SLQueryResult execute( String[] inputNodesIDs,
        ...
        try {
            ...
            SLSelectStatementInfo lastSelectInfo = getLastSelect();
            ...
            for (SLSelect select : selects) {
                ...
                resultSelectNodeWrappers = applyLimitOffset(resultSelectNodeWrappers, selectStatementInfo.getLimit(), selectStatementInfo.getOffset());
    ...
    private Collection<PNodeWrapper> applyLimitOffset( final Collection<PNodeWrapper> input,
        ...
        LinkedList<PNodeWrapper> resultList = new LinkedList<PNodeWrapper>();

        int m_limit = limit;
        int m_offset = offset == null ? 0 : offset - 1;
        int i = 0;
        int addedElements = 0;
        for (PNodeWrapper pNodeWrapper : input) {
            if (i >= m_offset) {
    ...
    private Collection<PNodeWrapper> getResultCollection( SLSelectStatementInfo selectStatementInfo,
                                                          SortMode sortMode ) {
        SLOrderByStatementInfo orderByStatementInfo = selectStatementInfo.getOrderByStatementInfo();
     ...
     */
    private void print( boolean showSLQL,
                        Object object ) {
        if (showSLQL) {
            LOGGER.info(object);
        }
     ...
     */
    private void validateSelectStatementInfoBeforeNormalization( SLSelectStatementInfo selectInfo )
     ...
     * @throws SLInvalidQueryElementException the SL invalid query element exception
     */
    private void validateSelectStatementInfoAfterNormalization( SLSelectStatementInfo selectInfo )
     ...
     * @throws SLGraphSessionException the SL graph session exception
     */
    private void validateTypesExsistence( SLSelectStatementInfo selectInfo )
     ...
     * @throws SLInvalidQuerySyntaxException the SL invalid query syntax exception
     */
    private void validateLinkTypeStatementInfo( SLLinkTypeStatementInfo linkTypeStatementInfo )
        throws SLInvalidQuerySyntaxException {
     ...
     * @throws SLGraphSessionException the SL graph session exception
     */
    private Collection<SLMetaNodeType> getMetaNodeTypes( String name,
                                                         boolean subTypes ) throws SLGraphSessionException {
    ...
    private Comparator<PNodeWrapper> getPNodeWrapperComparator() {
        return new Comparator<PNodeWrapper>() {
            public int compare( PNodeWrapper nodeWrapper1,
                                PNodeWrapper nodeWrapper2 ) {
                try {
    ...
    private Comparator<PNodeWrapper> getOrderByPNodeWrapperComparator( final SLOrderByStatementInfo orderByStatementInfo ) {
        return new Comparator<PNodeWrapper>() {
            ...
            private int normalizeCompareValue( int value,
                                               OrderType orderType ) {
            ...
            private boolean isInstanceOf( String subTypeName,
                                          String typeName ) throws SLGraphSessionException {
            ...
            private boolean isInstanceOf( String subTypeName,
                                          SLMetaNodeType metaNodeType ) throws SLGraphSessionException {
...
class PNodeWrapper {
    ...
    PNodeWrapper(
                  SLPersistentNode pNode, String typeName ) {
        this.pNode = pNode;
