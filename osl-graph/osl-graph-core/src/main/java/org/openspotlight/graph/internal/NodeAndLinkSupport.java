...
public class NodeAndLinkSupport {
	...
	private static <T extends Node> T createNode(
		...
		if (session != null) {
			...
			internalNodeKey = new NodeKeyBuilderImpl(targetNodeType.getName(),
					partition, repositoryPath).withSimpleKey(NAME, name)
					.andCreate();
		}
		...
		if (node != null) {
			internalNode.cachedEntry = new WeakReference<org.openspotlight.storage.domain.StorageNode>(
					node);
	...
	public static org.openspotlight.storage.domain.StorageNode retrievePreviousNode(
			final PartitionFactory factory, final StorageSession session,
			final Context context, final Node node,
			final boolean needsToVerifyType) {
		try {
			final PropertyContainerMetadata<org.openspotlight.storage.domain.StorageNode> metadata = (PropertyContainerMetadata<org.openspotlight.storage.domain.StorageNode>) node;
			org.openspotlight.storage.domain.StorageNode internalNode = metadata
					.getCached();
	...
	public static <T extends Link> T createLink(final PartitionFactory factory,
		...
		String linkId = null;
		if (session != null) {
			final org.openspotlight.storage.domain.StorageNode originAsSTNode = session
					.findNodeByStringId(origin.getId());
			final org.openspotlight.storage.domain.StorageNode targetAsSTNode = session
			...
			linkId = StringIDSupport.getLinkKeyAsString(
					StringIDSupport.getPartition(origin.getId(), factory),
					clazz.getName(), origin.getId(), target.getId());
		}
	...
	public static <T extends Node> T createNode(final PartitionFactory factory,
			final StorageSession session, final String contextId,
			final String parentId, final Class<T> clazz, final String name,
			final boolean needsToVerifyType,
			final Iterable<Class<? extends Link>> linkTypesForLinkDeletion,
			final Iterable<Class<? extends Link>> linkTypesForLinkedNodeDeletion) {
	...
	private static class LinkImpl extends Link
			implements
			PropertyContainerMetadata<org.openspotlight.storage.domain.StorageLink>,
		...
		@Override
		public void setCached(org.openspotlight.storage.domain.StorageLink entry) {
			cachedEntry = new WeakReference<org.openspotlight.storage.domain.StorageLink>(
					entry);
	...
	private static class NodeImpl extends Node
			implements
			PropertyContainerMetadata<org.openspotlight.storage.domain.StorageNode>,
			PropertyContainerLineReferenceData {
		...
		public void setCached(
			...
			cachedEntry = new WeakReference<org.openspotlight.storage.domain.StorageNode>(
					entry);
