...
public abstract class BaseTask implements Task {
    ...
    private static int countDependencies(Class<? extends Task> thisType) {
        StrongDependsOn strongDeps = thisType.getAnnotation(StrongDependsOn.class);
        WeakDependsOn weakDeps = thisType.getAnnotation(WeakDependsOn.class);
        Salience salience = thisType.getAnnotation(Salience.class);
        int salienceValue = salience != null ? salience.value() : 0;
        int weakDepsCount = weakDeps != null ? countDependencies(weakDeps.value()) : 0;
        int strongDepsCount = strongDeps != null ? countDependencies(strongDeps.value()) : 0;
