...
public class OslContextListener implements ServletContextListener,
	...
	public void contextDestroyed(final ServletContextEvent arg0) {
		...
		// OslServletContextSupport.getJcrConnectionFrom(arg0.getServletContext());
		// final SLGraph graph =
		// OslServletContextSupport.getGraphFrom(arg0.getServletContext());
		// // final Scheduler scheduler =
		// OslServletContextSupport.getSchedulerFrom(arg0.getServletContext());
		// graph.shutdown();
	...
	public void contextInitialized(final ServletContextEvent arg0) {
		try {
			// //FIXME create a login process and a per user configuration
			// stored in a web session
			// final DefaultJcrDescriptor descriptor =
			// DefaultJcrDescriptor.DEFAULT_DESCRIPTOR;
			// final SecurityFactory securityFactory =
			// AbstractFactory.getDefaultInstance(SecurityFactory.class);
			// final User simpleUser = securityFactory.createUser("testUser");
			// final AuthenticatedUser user =
			// securityFactory.createIdentityManager(descriptor).authenticate(simpleUser,
			// "password");
			//
			// final JcrConnectionProvider provider =
			// JcrConnectionProvider.createFromData(descriptor);
			// final SLGraphFactory factory =
			// AbstractFactory.getDefaultInstance(SLGraphFactory.class);
			...
			// jcrSession.logout();
			// final JcrConfigurationManagerProvider
			// configurationManagerProvider = new
			// JcrConfigurationManagerProvider(provider);
			// scheduler.setBundleProcessorManager(new
			// BundleProcessorManagerImpl(user, provider,
			...
			// arg0.getServletContext().setAttribute(JCR_DESCRIPTOR,
			// descriptor);
