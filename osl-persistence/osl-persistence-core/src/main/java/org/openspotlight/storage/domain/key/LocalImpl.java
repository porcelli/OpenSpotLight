...
public class LocalImpl implements LocalKey {
    ...
    public LocalImpl(final Set<Key> entries, final String nodeEntryName) {
        ...
        final List<Key> tempEntries = new ArrayList<Key>(entries);
        sort(tempEntries);
    ...
    public boolean equals(final Object o) {
        ...
        final LocalImpl localKey = (LocalImpl) o;
    ...
    @Override
    public int compareTo(final LocalKey o) {
        int result = getNodeEntryName().compareTo(o.getNodeEntryName());
        if (result != 0) { return result; }
        final Iterator<Key> thisIt = getEntries().iterator();
        final Iterator<Key> thatIt = o.getEntries().iterator();
        while (true) {
            final boolean thisHasNext = thisIt.hasNext();
            final boolean thatHasNext = thatIt.hasNext();

            if (thisHasNext && thatHasNext) {
                final Key thisNext = thisIt.next();
                final Key thatNext = thatIt.next();
