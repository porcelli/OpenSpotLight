...
public class MongoSTStorageSessionImpl extends AbstractSTStorageSession<DBObject> {
    ...
    protected void internalSavePartitions( STPartition... partitions ) throws Exception {
        for (STPartition partition : partitions) {
            for (Pair<STNodeEntry, DBObject> p : transientObjects.get(partition)) {
                DBCollection coll = getCachedCollection(partition, p.getK1().getNodeEntryName());
                coll.save(p.getK2());
            }
        }
    ...
    @Override
    protected Iterable<STNodeEntry> internalFindByCriteria( final STPartition partition,
                                                            final STCriteria criteria ) throws Exception {
    ...
    @Override
    protected void flushRemovedItem( STPartition partition,
                                     STNodeEntry entry ) throws Exception {
        DBCollection collection = getCachedCollection(partition, entry.getNodeEntryName());
        collection.remove(new BasicDBObject(ID, entry.getUniqueKey().getKeyAsString()));
    }

    @Override
    protected Iterable<STNodeEntry> internalNodeEntryGetNamedChildren( STPartition initialPartition,
    ...
    private Iterable<STNodeEntry> internalGetChildren( final STPartition partition,
                                                       STNodeEntry stNodeEntry,
                                                       String name ) throws Exception {
    ...
    protected void internalFlushSimpleProperty( DBObject possibleReference,
                                                STPartition partition,
                                                STProperty dirtyProperty ) throws Exception {
        ...
        if (dirtyProperty.isIndexed()) {
            ensureIndexed(partition, dirtyProperty.getParent().getNodeEntryName(), INDEXED, dirtyProperty.getPropertyName());
            objName = INDEXED;
    ...
    public void storeInGridFS( STPartition partition,
                               STProperty property,
    ...
    private String getFileName( STPartition partition,
                                STProperty property ) {
        String key = partition.getPartitionName() + _ + property.getParent().getUniqueKey().getKeyAsString() + _ + property.getPropertyName();
        return key;
    }

    public byte[] readAsGridFS( STPartition partition,
                                STProperty property ) throws Exception {
        String key = getFileName(partition, property);
        GridFS fs = getCachedGridFSForPartition(partition);
    ...
    private void ensureIndexed( STPartition partition,
                                String parentName,
                                String groupName,
                                String propertyName ) {
        String key = partition.getPartitionName() + parentName + groupName + propertyName;
        if (!allIndexes.contains(key)) {
            allIndexes.add(key);
            getCachedCollection(partition, parentName).ensureIndex(groupName != null ? (groupName + "." + propertyName) : propertyName);
        }
    }

    @Override
    protected Iterable<STNodeEntry> internalNodeEntryGetChildren( STPartition partition,
    ...
    @Override
    protected Set<STProperty> internalNodeEntryLoadProperties( DBObject possibleReference,
                                                               STPartition partition,
                                                               STNodeEntry stNodeEntry ) throws Exception {
    ...
    private STNodeEntry convertToNode( STPartition partition,
                                       DBObject dbObject ) throws Exception {
