...
public enum RedisServerExecutor {
    ...
    private static final String ERROR_MESSAGE         =
                                                          "It is necessary to newPair an environment variable or JVM property for OSL_HOME ."
                                                              +
    ...
    public synchronized void startServerIfNecessary(final Partition somePartition,
        ...
        if (currentProcess == null) {
            ...
            try {
                if (redisHome == null) {
                    redisHome = concatPaths(oslHome, RELATIVE_REDIS_PATH);
                }
                final String redisExec = concatPaths(redisHome, REDIS_EXEC);
                if (!new File(redisExec).exists()) {
                    final Process make = Runtime.getRuntime().exec(MAKE, null, new File(redisHome));
                    make.waitFor();
                    if (!new File(redisExec).exists()) { throw new IllegalStateException(ERROR_COMPILING_REDIS); }
                }
                ...
                Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
                    ...
                    public void run() {
                        if (currentProcess != null) {
                            try {
                                final JRedis redis = factory.getFrom(somePartition);
                                redis.save();
                                redis.shutdown();
                                currentProcess = null;
