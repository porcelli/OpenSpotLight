...
public class JRedisStorageSessionImpl extends AbstractStorageSession<Nothing> {
	...
	private class JRedisLoggedExecution {
		...
		public void sadd(final String s, final String o) throws Exception {
			jredis.sadd(s, o);
			if (!s.contains(uuid)) {
		...
		public void set(final String s, final String o) throws Exception {
			jredis.set(s, o);
			if (!s.contains(uuid)) {
				jredis.sadd(usedKeys, s);
			}
		...
		public void set(final String s, final byte[] o) throws Exception {
			jredis.set(s, o);
			if (!s.contains(uuid)) {
				jredis.sadd(usedKeys, s);
			}
	...
	private StorageNode loadNodeOrReturnNull(final String firstParentKey,
			final Partition firstPartition) throws Exception {
		...
		NodeKeyBuilder keyBuilder = withPartition(firstPartition).createKey(
				getNodeEntryName(firstParentKey));
	...
	protected void flushRemovedLink(final Partition partition,
			final StorageLink link) throws Exception {
		final JRedis jredis = factory.getFrom(partition);
	...
	protected Iterable<StorageLink> internalFindLinks(
			...
			final StorageNode destiny, final String name) throws Exception {
		final JRedis jredis = factory.getFrom(partition);
		...
		return IteratorBuilder.<StorageLink, String> createIteratorBuilder()
				...
				.withConverter(new Converter<StorageLink, String>() {
					...
					public StorageLink convert(final String o) throws Exception {
						...
						return new StorageLinkImpl(StringIDSupport
	...
	protected Set<Property> internalPropertyContainerLoadProperties(
			...
			final PropertyContainer StorageNode) throws Exception {
		final JRedis jredis = factory.getFrom(partition);
