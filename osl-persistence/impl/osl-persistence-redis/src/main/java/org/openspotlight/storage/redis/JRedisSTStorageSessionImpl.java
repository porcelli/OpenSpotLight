...
public class JRedisSTStorageSessionImpl extends
	...
	protected byte[] internalPropertyGetValue(STPartition partition,
		...
		String uniqueKey = stProperty.getParent().getUniqueKey()
				.getKeyAsString();
	...
	protected Set<STProperty> internalNodeEntryLoadProperties(
			Nothing reference, STPartition partition, STNodeEntry stNodeEntry)
			throws Exception {
		JRedis jredis = factory.getFrom(partition);
	...
	private STProperty loadProperty(STPartition partition,
			STNodeEntry stNodeEntry, String parentKey, String propertyName,
	...
	protected void internalFlushSimpleProperty(Nothing reference,
		...
		JRedis jredis = factory.getFrom(partition);
		String uniqueKey = dirtyProperty.getParent().getUniqueKey()
				.getKeyAsString();
