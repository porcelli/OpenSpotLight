...
public enum ExecutorInstance {
    ...
    public <T> List<Future<T>> invokeAll(final Collection<Callable<T>> itemsToExecute) {
        try {
            final int threads = 2 * Runtime.getRuntime().availableProcessors();
            final GossipExecutor executor =
                GossipExecutor.newFixedThreadPool(threads, "temporary-pool-" + UUID.randomUUID().toString());
