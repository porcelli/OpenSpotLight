...
/**
 * Class to be used as a hierarchy to priorities. One example is under thread pool task executor. Imagine that a processor
 * executes tasks with priorities. It will execute first the task one, and after task two, and so on. Sometimes should be good to
 * execute task 2.1 before task 3 and after task 2. This behavior can be achieved here by instantiating a
...
public final class Priority implements Comparable<Priority> {
    ...
    private Priority(
                      final int priorityNumber, final Priority subPriority) {
    ...
    public int compareTo(final Priority o) {
        ...
        while (true) {
            ...
            if (thisPriority == null && thatPriority == null) { return 0; }
            if (thisPriority == null || thatPriority == null) {
                if (thisPriority == null) { return compareIntValues(1, 0); }
