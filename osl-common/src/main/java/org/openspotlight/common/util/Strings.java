...
public class Strings {
    ...
    public static <T> String bigCollectionsToString(final Collection<T> col) {
        ...
        if (size >= 2) {
            second = it.next();
            if (size == 2) { return "[" + first + ", " + second + "]"; }
        }
        third = it.next();
        if (size == 3) { return "[" + first + ", " + second + ", " + third + "]"; }
    ...
    public static String firstLetterToLowerCase(final String toBeCorrected) {
        ...
        if (toBeCorrected.length() == 0) { return toBeCorrected; }
        if (toBeCorrected.length() == 1) { return toBeCorrected.toLowerCase(); }
        final String newString = toBeCorrected.substring(0, 1).toLowerCase() + toBeCorrected.substring(1);
        return newString;
    }
    ...
    public static String firstLetterToUpperCase(final String toBeCorrected) {
        checkNotNull("toBeCorrected", toBeCorrected);//$NON-NLS-1$
        if (toBeCorrected.length() == 0) { return toBeCorrected; }
        if (toBeCorrected.length() == 1) { return toBeCorrected.toUpperCase(); }
        final String newString = toBeCorrected.substring(0, 1).toUpperCase() + toBeCorrected.substring(1);
        return newString;
    }
    ...
    public static String replaceLast(final String toChange,
        ...
        if (toChange.indexOf(toBeReplaced) == -1) { return toChange; }
    ...
    public static String rootPath(final String paths) {
        if (paths.startsWith("/")) { return paths; }
        return "/" + paths;
    }
