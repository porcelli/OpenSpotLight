...
public class SimplePersistImpl implements SimplePersistCapable<STNodeEntry, STStorageSession> {
    ...
    private <T> void fillChildrenProperties(ConversionToNodeContext context, T bean,
        ...
        for (BeanToNodeChildData data : nodesToConvert.values()) {
            ...
            Set<STNodeEntry> unusedChildren = new HashSet(newNodeEntry.getChildrenNamed(context.session, internalGetNodeName(data.nodeType)));
    ...
    private Object internalConvertNodeToBean(ConversionToBeanContext context, STNodeEntry nodeEntry, Object beanParent) throws Exception {
        ...
        if (nodeEntry == null) {
            ...
            if (cached == null) {
                Class<?> beanType = forName(internalGetNodeType(context.node));
                cached = beanType.newInstance();

                Descriptors descriptors = Descriptors.fillDescriptors(beanType);
                fillParents(context.session, nodeEntry, cached, descriptors.parentPropertiesDescriptor, beanParent);
