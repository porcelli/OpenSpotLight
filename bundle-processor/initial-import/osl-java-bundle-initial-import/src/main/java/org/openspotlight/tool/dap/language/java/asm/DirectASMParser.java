...
public class DirectASMParser {
	...
	public DirectASMParser(String type) {
	...
	public MethodDeclaration method(String name, boolean isConstructor) {
		return mMETHOD(name, isConstructor);
	}

	private MethodDeclaration mMETHOD(String name, boolean isConstructor) {
	...
	private TypeRef mOBJECT() {
		...
		Pair<String, String> packageAndClassName = separatePackage(nameList
				.get(0));
		nameList.remove(0);

		rootType = buildType(packageAndClassName.getK1(), packageAndClassName
				.getK2(), nameList, genericTypeList);
	...
	private Pair<String, String> separatePackage(Queue<String> queue) {
		...
		String packageName = tempValue.substring(0, tempValue.lastIndexOf('/'))
				.replace("/", ".");
	...
	private TypeRef buildType(String packageName, String typeName,
			List<Queue<String>> nameList, List<List<TypeRef>> genericTypeList) {
		...
		if (genericTypeList.size() > 0 && genericTypeList.get(0) != null) {
			resultType = new ParameterizedTypeRef(genericTypeList.get(0),
					resultType);
		}
		...
		for (int a = 0; a < nameList.size(); a++) {
			...
			for (int i2 = 0; i2 < nameList.get(a).size(); i2++) {
				resultType = new QualifiedTypeRef(resultType, nameList.get(a)
						.poll());
			}
			if (genericTypeList.size() > a + 1
					&& genericTypeList.get(a + 1) != null) {
				resultType = new ParameterizedTypeRef(genericTypeList
						.get(a + 1), resultType);
			}
		}
