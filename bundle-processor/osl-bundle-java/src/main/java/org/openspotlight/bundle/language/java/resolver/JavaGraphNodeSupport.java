 ...
 * instead of inserting nodes and links by hand. This class creates all
 * necessary links and nodes, includding the implicit ones. When receiving int
 * parameters on access parameters inside methods, the {@link Opcodes} constants
 * from ASM should be used. First of all, all types on the current classpath
...
public class JavaGraphNodeSupport {
	 ...
	 * will store all implicit relationships and also all abstract types will be
	 * there. The concrete types will be created on currentContextRootNode. The
	 * abstractContextRootNode should be used as a "global classpath". First
	 * thing to use this class in a proper way: creates all types declared in
	 * current classpath on the currentContextRootNode. When adding new implicit
	 * types (on throws, or parameter, or field declarations for example) this
	 * types will be created on the abstract context in case where this types
	 * doesn't exists on current context.
	 * 
	 * @param session
	 *            the session
	 * @param currentContextRootNode
	 *            the current context root node
	 * @param abstractContextRootNode
	 *            the abstract context root node
	...
	private void addImplicitPrimitiveCast(final String typeName,
			final String superTypeName, final int distance) throws Exception {
	...
	/**
	 * Adds the type on abstract context to be used as a
	 * "global classphath element". The types declared on current classpath
	 * should be added with method
	 * {@link #addTypeOnCurrentContext(Class, String, String, int)}
	 * 
	 * @param nodeType
	 *            the node type
	 * @param packageName
	...
	/**
	 * Adds the type on current context. This method should be used on current
	 * classpath elements only. The types declared outside this classpath should
	 * be added with method
	 * {@link #addTypeOnAbstractContext(Class, String, String)}.
	 * 
	 * @param nodeType
	 *            the node type
	 * @param packageName
	 ...
	 *            the access
	 * @param parentType
	 *            the parent type, if null will use package as parent
	 * @return the t
	 * @throws Exception
	 *             the exception
	 */
	...
	/**
	 * Creates the method and also its links. The parameters, exceptions and so
	 * on should be created with the proper methods on this class.
	 * 
	 * @param newType
	 *            the new type
	...
	private void insertFieldData(final JavaDataField field,
			final JavaType fieldType, final int access, final boolean isArray,
	...
	private void setupWrapperAndPrimitive(final String primitiveName,
			final String wrapperPackage, final String wrapperClass)
			throws Exception {
