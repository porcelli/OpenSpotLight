 ...
 * exported from jcr was to much dirty for using as a simple configuration. LATER_TASK implement node property
 * 
...
public class XmlConfigurationManager implements ConfigurationManager {
    ...
    /**
     * Default constructor that receives a file url and a boolean to mark the artifacts as an ignored node. This should be useful
     * to use this as a simple configuration and not to store all the artifact metadata.
     * 
     * @param url
     */
    ...
    private void createEachXmlNode( final Element element,
        ...
        final Map<String, Class<?>> propertyTypesMap = map(ofKeys(propKeys), andValues(propValues));
        for (final Map.Entry<String, Object> propertyEntry : properties.entrySet()) {
            if (!propertyEntry.getKey().equals("")) { //$NON-NLS-1$
                if (Serializable.class.isAssignableFrom(propertyTypesMap.get(propertyEntry.getKey()))) {
                    this.setPropertyOnXml(element, propertyEntry.getKey(), (Serializable)propertyEntry.getValue(),
                                          propertyTypesMap.get(propertyEntry.getKey()));
                }
            }
        }
        for (final Class<? extends ConfigurationNode> configuredChildClass : configurationNode.getInstanceMetadata().getStaticMetadata().validChildrenTypes()) {
            ...
            final Set<Serializable> keys = (Set<Serializable>)configurationNode.getInstanceMetadata().getKeyFromChildrenOfTypes(
                                                                                                                                configuredChildClass);
            for (final Serializable key : keys) {
                final ConfigurationNode innerNode = configurationNode.getInstanceMetadata().getChildByKeyValue(
                                                                                                               (Class<? extends ConfigurationNode>)configuredChildClass,
                                                                                                               key);
                final Class<? extends ConfigurationNode> childClass = innerNode.getClass();
                final Element newElement = element.addElement(removeBegginingFrom(
                                                                                  "osl:", //$NON-NLS-1$
                                                                                  this.classHelper.getNameFromNodeClass(childClass)));
                if (!innerNode.getInstanceMetadata().getStaticMetadata().keyPropertyName().equals("")) { //$NON-NLS-1$
                    newElement.addAttribute(innerNode.getInstanceMetadata().getStaticMetadata().keyPropertyName(),
                                            convert(key, String.class));
                }
     ...
     * {@inheritDoc}
     */
    public Configuration load( final LazyType type ) throws ConfigurationException {
        checkNotNull("type", type); //$NON-NLS-1$
        checkCondition("typeEqualsLazy", LazyType.EAGER.equals(type)); //$NON-NLS-1$
        try {
            final Document document = this.inputStream == null ? this.reader.read(this.url) : this.reader.read(this.inputStream);
    ...
    private void loopOnEachElement( final Element parentElement,
        ...
        final String[] propKeys = parentNode.getInstanceMetadata().getStaticMetadata().propertyNames();
        final Class<?>[] propValues = parentNode.getInstanceMetadata().getStaticMetadata().propertyTypes();
        final Map<String, Class<?>> propertyTypes = map(ofKeys(propKeys), andValues(propValues));
        final StaticMetadata parentStaticMetadata = this.classHelper.getStaticMetadataFromClass(parentNode.getClass());
        final String parentKeyProperty = parentStaticMetadata.keyPropertyName();
        for (final Iterator<Attribute> properties = parentElement.attributeIterator(); properties.hasNext();) {
            ...
            this.setPropertyOnNode(parentNode, propertyName, propertyTypes.get(propertyName), valueAsString);
        }
        for (final Iterator<Element> elements = parentElement.elementIterator(); elements.hasNext();) {
            final Element nextElement = elements.next();
            final String nodeClassName = nextElement.getName();
            final Class<? extends ConfigurationNode> nodeClass = this.classHelper.getNodeClassFromName("osl:" + nodeClassName); //$NON-NLS-1$
            if (GeneratedNode.class.isAssignableFrom(nodeClass)) {
                continue;
            }
            final StaticMetadata staticMetadata = this.classHelper.getStaticMetadataFromClass(nodeClass);
            final String keyProperty = staticMetadata.keyPropertyName();
            final ConfigurationNode newNode;
            if (!"".equals(keyProperty)) { //$NON-NLS-1$
                final String keyPropertyValueAsString = nextElement.attributeValue(keyProperty);
                final Serializable keyPropertyValue = convert(keyPropertyValueAsString, staticMetadata.keyPropertyType());
                newNode = this.classHelper.createInstance(keyPropertyValue, parentNode, "osl:" + nodeClassName); //$NON-NLS-1$
            } else {
                newNode = this.classHelper.createInstance(null, parentNode, "osl:" + nodeClassName); //$NON-NLS-1$
            }
    ...
    public void save( final Configuration configuration ) throws ConfigurationException {
        checkCondition("urlNotNull", this.url != null);
        try {
            if (this.url.indexOf("/") != -1) { //$NON-NLS-1$
                final String dir = this.url.substring(0, this.url.lastIndexOf("/")); //$NON-NLS-1$
                new File(dir).mkdirs();
            }
            ...
            final OutputStream os = new BufferedOutputStream(new FileOutputStream(this.url));
    ...
    private void setPropertyOnNode( final ConfigurationNode newNode,
        ...
        if (valueAsString == null) {
            ...
            newNode.getInstanceMetadata().setProperty(propertyName, Boolean.valueOf(valueAsString));
        } else if (Double.class.equals(propertyClass)) {
            newNode.getInstanceMetadata().setProperty(propertyName, Double.valueOf(valueAsString));
        } else if (Long.class.equals(propertyClass)) {
            newNode.getInstanceMetadata().setProperty(propertyName, Long.valueOf(valueAsString));
        } else if (String.class.equals(propertyClass)) {
            newNode.getInstanceMetadata().setProperty(propertyName, valueAsString);
        } else if (Integer.class.equals(propertyClass)) {
            newNode.getInstanceMetadata().setProperty(propertyName, Integer.valueOf(valueAsString));
        } else if (Byte.class.equals(propertyClass)) {
            newNode.getInstanceMetadata().setProperty(propertyName, Byte.valueOf(valueAsString));
        } else if (Float.class.equals(propertyClass)) {
            newNode.getInstanceMetadata().setProperty(propertyName, Float.valueOf(valueAsString));
        } else if (Date.class.equals(propertyClass)) {
            newNode.getInstanceMetadata().setProperty(propertyName, dateFromString(valueAsString));
        } else if (propertyClass.isEnum()) {
            final Field[] flds = propertyClass.getDeclaredFields();
            for (final Field f : flds) {
                if (f.isEnumConstant()) {
                    if (f.getName().equals(propertyName)) {
                        final Serializable newValue = (Serializable)f.get(null);
                        newNode.getInstanceMetadata().setProperty(propertyName, newValue);
                        break;
                    }
                }
            }
    ...
    private void setPropertyOnXml( final Element newElement,
                                   final String key,
                                   final Serializable value,
                                   final Class<?> propertyClass ) throws SLException {
        if (value == null) {
