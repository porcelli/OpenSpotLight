...
import org.openspotlight.bundle.domain.GroupListener;
import org.openspotlight.bundle.domain.Repository;
import org.openspotlight.bundle.domain.GroupListener.ListenerAction;
import org.openspotlight.bundle.domain.Repository.GroupVisitor;
import org.openspotlight.federation.domain.*;
import org.openspotlight.bundle.processing.BundleExecutionException;
...
import org.openspotlight.bundle.processing.internal.task.SaveGraphTask;
import org.openspotlight.bundle.processing.internal.task.StartingToSearchArtifactsTask;
            ...
            for (final Repository repository : repositories) {
                ...
                for (final GroupListener instance : groupListenerInstances) {
                    ...
                    for (final String added : differences.getAddedGroups()) {
                        final Node groupAsNode = groupContext.getRootNode().getNode(added);
                        try {
                            final ListenerAction response = instance.groupAdded(groupAsNode, internalContext);
                    ...
                    for (final String removed : differences.getRemovedGroups()) {
                        final Node groupAsNode = groupContext.getRootNode().getNode(removed);
                        try {
                            final ListenerAction response = instance.groupRemoved(groupAsNode, internalContext);
                            if (ListenerAction.IGNORE.equals(response)) {
                                continue removing;
                            }
                        } catch (final Exception e) {
