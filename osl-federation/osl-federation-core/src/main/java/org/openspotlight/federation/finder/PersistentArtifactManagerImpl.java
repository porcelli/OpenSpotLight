...
public class PersistentArtifactManagerImpl extends
    ...
    protected <A extends Artifact> void internalAddTransient(A artifact)
        ...
        simplePersist.convertBeanToNode(this.rootPath,
                artifact);
    }
    ...
    private <A> A internalFind(Class<A> type,
                               ...
                               String propertyName)
            throws Exception {
        return simplePersist.findUniqueByProperties(
                this.rootPath, type,
    ...
    protected <A extends Artifact> void internalMarkAsRemoved(A artifact)
            throws Exception {
        final STNodeEntry node = simplePersist.convertBeanToNode(this.rootPath,
                artifact);
    ...
    private <A> Set<String> privateRetrieveNames(Class<A> type,
        ...
        for (STNodeEntry nodeEntry : foundNodes) {
            names.add(nodeEntry.<String>getPropertyValue(simplePersist.getCurrentSession(), propertyName));
        }
