...
public class LocalSourceArtifactFinderByRepositoryProvider implements
	...
	public synchronized <A extends Artifact> ArtifactFinder<A> getByRepository(
		...
		if (StreamArtifact.class.equals(artifactType)) {
			if (useChangeFolders) {
				return (ArtifactFinder<A>) new LocalSourceStreamArtifactFinder(
						artifactSource);
			} else {
				return (ArtifactFinder<A>) new FileSystemStreamArtifactFinder(
						artifactSource);
			}
		} else {
			throw Exceptions
					.logAndReturn(new IllegalArgumentException(
