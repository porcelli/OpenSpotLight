...
public class ArtifactFinderSupport {
	 ...
	 * @param artifact
	 *            the artifact
	 * @param setWithEquivalent
	 *            the set with equivalent
	 * @return the t
	 */
	...
	/**
	 * Freeze changes after bundle processing. This will remove all artifacts
	 * marked as excluded and change all other status to not changed.
	 * 
	 * @param existents
	 *            the existents
	 */
	public static <T extends Artifact> void freezeChangesAfterBundleProcessing(
			final Set<T> existents, PersistentArtifactManager manager) {
		...
		if (manager != null) {
			Iterable<StorageNode> nodesToExclude = manager.getSimplePersist()
					.convertBeansToNodes(toBeRemoved);
			for (StorageNode nodeToExclude : nodesToExclude) {
				manager.getSimplePersist().getCurrentSession()
						.removeNode(nodeToExclude);
			}
