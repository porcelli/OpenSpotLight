 ...
 * processing them or creating the sha-1 key for the content. This is working now as a multi threaded artifact loader also.
 * 
...
public abstract class AbstractArtifactLoader implements ArtifactLoader {
    ...
    /**
     * Return type for method {@link AbstractArtifactLoader#createErrorHandler()}. This class will receive callbacks on each error
    ...
    protected interface ArtifactErrorHandler {
        ...
        /**
         * An erro processing one artifact just happened, but it will not stop the artifact processing.
         * 
         * @param bundle the bundle
     ...
     * @author Luiz Fernando Teston - feu.teston@caravelatech.com
     */
    protected final static class DefaultErrorHandler implements ArtifactErrorHandler {
        /**
         * {@inheritDoc}
         */
        public void fatalErrorHappened( final ArtifactSource bundle,
     ...
     * @author Luiz Fernando Teston - feu.teston@caravelatech.com
     */
    protected abstract static class DefaultGlobalExecutionContext implements GlobalExecutionContext {
     ...
     * @author Luiz Fernando Teston - feu.teston@caravelatech.com
     */
    protected abstract static class DefaultThreadExecutionContext implements ThreadExecutionContext {
        /**
         * {@inheritDoc}
         */
    ...
    /**
     * Durring the artifact processing, there cold exists some resources to initialize durring the global processing phases. A
     * single instance of this class will be created and it will be shared by all threads and calls during the artifact loading
     * process. This interface is the return type of method {@link AbstractArtifactLoader#createGlobalExecutionContext()} If
     * single thread resources initialization is needed, use the {@link ContentExecutionContext} instead.
     * 
     * @author Luiz Fernando Teston - feu.teston@caravelatech.com
     */
    @NotThreadSafe
    protected static interface GlobalExecutionContext {
        ...
        /**
         * The implementation class needs to load all the possible artifact names without filtering this.This method is
         * "mono threaded", so it's not dangerous to do something non multi-threaded here.
         * 
         * @param bundle the bundle
         * @param mapping the mapping
        ...
        /**
         * This method will be called when the artifact processing is about to start.
         * 
         * @param bundle the bundle
         */
        public void globalExecutionAboutToStart( final ArtifactSource bundle );
    ...
    /**
     * Durring the artifact processing, there cold exists some resources to initialize durring the starting and stopping phases of
     * a single thread. A single instance of this class will be created for each thread and it won't be shared by all threads. The
     * same {@link ContentExecutionContext} will be shared by various artifact loading invocation on a single thread. This
     * interface is the return type of method {@link AbstractArtifactLoader#createThreadExecutionContext()} If global resources
     * initialization is needed, use the {@link GlobalExecutionContext} instead.
     * 
     * @author Luiz Fernando Teston - feu.teston@caravelatech.com
     */
    @ThreadSafe
    protected static interface ThreadExecutionContext {
        ...
        /**
         * This method should do the loading processing itself. It will be done on a multi threaded execution. This will be called
         * in a lazy way. In case of a null returning, the {@link ArtifactLoader} will consider that all the work to add a
         * metadata on the repository is already done.
         * 
         * @param bundle the bundle
         * @param mapping the mapping
    ...
    private final class Worker implements Callable<Void> {
         ...
         * @param loadCounter the load counter
         * @param mapping the mapping
         * @param namesToProcess the names to process
         * @param globalContext the global context
        ...
        public Void call() {
            ...
            AbstractArtifactLoader.this.loadArtifactsOfNames(this.bundle, this.errorCounter, this.loadCounter, this.mapping,
    ...
    /**
     * Overwrite this method to create a custom {@link ArtifactErrorHandler} to be used on a {@link AbstractArtifactLoader}
     * extended class.
     * 
    ...
    /**
     * Overwrite this method to create a custom {@link GlobalExecutionContext} to be used on a {@link AbstractArtifactLoader}
     * extended class.
     * 
     * @return an {@link ArtifactErrorHandler}
     */
    ...
    /**
     * Overwrite this method to create a custom {@link ThreadExecutionContext} to be used on a {@link AbstractArtifactLoader}
     * extended class.
     * 
     * @return an {@link ArtifactErrorHandler}
     */
    ...
    /**
     * Some artifact loaders needs different behavior to create the mapping string to be used on pattern matchers. So, this method
     * should be overwritten in this cases.
     * 
    ...
    /**
     * Filter the included and excluded patterns and also creates each artifact and calculates the sha-1 key for the content. In
     * this method we have also the logic for dividing the tasks between {@link Configuration#getNumberOfParallelThreads() the
     * number of parallel threads}.
     * 
     * @param bundle the bundle
     * @return a {@link ArtifactLoader.ArtifactProcessingCount} with statistical data
     * @throws ConfigurationException the configuration exception
     */
    ...
    public final ArtifactProcessingCount loadArtifactsFromMappings( final ArtifactSource bundle ) throws ConfigurationException {
        ...
        final GlobalExecutionContext globalContext = this.createGlobalExecutionContext();
        ...
        try {
            ...
            for (final ArtifactMapping mapping : bundle.getArtifactMappings()) {
                ...
                final FilterResult innerResult = filterNamesByPattern(namesToFilter, includedPatterns, excludedPatterns, false);
                final Set<String> namesToProcess = innerResult.getIncludedNames();
                final Set<String> names = new HashSet<String>(bundle.getStreamArtifactNames());
                ...
                for (final String name : bundle.getCustomArtifactNames()) {
                    ...
                    if (!found) {
                        final CustomArtifact artifactToDelete = bundle.getCustomArtifactByName(name);
                ...
                this.splitWorkBetweenThreads(bundle, errorCounter, loadCounter, mapping, namesToProcess, workers, globalContext,
                                             errorHandler);

            }
            try {
                final ExecutorService executor = newFixedThreadPool(globalContext.withThreadPoolSize(bundle));
        ...
        return new ArtifactProcessingCount(loadCounter.get(), ignoreCount, errorCounter.get());
    }

    /**
     * This method will be called by multi threads to load all artifacts of the given names.
     * 
     * @param bundle the bundle
     * @param errorCounter the error counter
     * @param loadCounter the load counter
    ...
    final void loadArtifactsOfNames( final ArtifactSource bundle,
                                     ...
                                     final GlobalExecutionContext globalContext,
                                     final ArtifactErrorHandler errorHandler ) {
        final ThreadExecutionContext localContext = this.createThreadExecutionContext();

        localContext.threadExecutionAboutToStart(bundle, mapping, globalContext);
        try {
            for (final String artifactName : namesToProcess) {
                try {

                    final byte[] content = localContext.loadArtifactOrReturnNullToIgnore(bundle, mapping, artifactName,
                                                                                         globalContext);
                    if (content == null) {
                        continue;
                    }
            ...
            localContext.threadExecutionFinished(bundle, mapping, globalContext);
        }
    }

    /**
     * Here all work of loading artifacts that needs to be done will be shared by the configured number of threads. This method
     * also will split the threads without mixing diferent {@link ArtifactMapping artifact mappings} .
     * 
     * @param bundle the bundle
     * @param errorCounter the error counter
     * @param loadCounter the load counter
    ...
    private void splitWorkBetweenThreads( final ArtifactSource bundle,
                                          ...
                                          final GlobalExecutionContext globalContext,
                                          final ArtifactErrorHandler errorHandler ) throws ConfigurationException {
        final int numberOfThreads = bundle.getRepository().getConfiguration().getNumberOfParallelThreads();
        final int allNamesToProcessSize = namesToProcess.size();
        final int numberOfNamesPerThread = allNamesToProcessSize / numberOfThreads;
        ...
        for (final Map.Entry<Integer, Set<String>> entry : listsForThreads.entrySet()) {
            final Worker w = new Worker(bundle, errorCounter, loadCounter, mapping, entry.getValue(), globalContext, errorHandler);
            workers.add(w);
        }
