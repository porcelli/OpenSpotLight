...
public class RoutineArtifact extends CustomArtifact {
	...
	/**
	 * Enum to describe routine types. Its int values was taken from
	...
	public static enum RoutineType {
		/**
		 * Type to describe a routine without return values from the routine
		 * itself. But it should have multiple out parameters.
		 */
		PROCEDURE(1),
		/**
		 * Type to descibe a routine with return value, but without out
		 * parameters.
		 */
		FUNCTION(2),
		/**
		 * Type to describe a routine on a situation that wasn't possible to
		 * retrieve the routine type.
		 */
		DONT_KNOW(0);
		...
		private RoutineType(final int sqlTypeValue) {
	 ...
	 * the format CATALOG_NAME/SCHEMA_NAME/<PROCEDURE|FUNCTION>/ROUTINE_NAME.
	 * Note that the /TABLE/ is a constant determining the artifact type.
	 * 
	 * @param bundle
	 * @param relativeName
	 */
	public RoutineArtifact(final Bundle bundle, final String relativeName) {
	...
	public final RoutineParameter getRoutineParameterByName(final String name) {
		return this.instanceMetadata.getChildByKeyValue(RoutineParameter.class,
				name);
	}
	...
	public final Set<String> getRoutineParameterNames() {
		return (Set<String>) this.instanceMetadata
				.getKeyFromChildrenOfTypes(RoutineParameter.class);
	}
	 ...
	 * @param routineColumn
	 */
	public final void removeRoutineParameter(
			final RoutineParameter routineColumn) {
