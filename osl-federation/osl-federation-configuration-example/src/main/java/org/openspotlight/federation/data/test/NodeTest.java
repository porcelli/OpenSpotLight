...
public class NodeTest {
    ...
    public void assertTheSameInitialDataOnSomeNodes( final Configuration configuration,
                                                     final boolean verifyArtifacts ) {
        ...
        assertThat(project.getArtifactSourceNames().size(), is(not(0)));
        assertThat(project.getArtifactSources().size(), is(not(0)));
        ...
        assertThat(innerProject.getArtifactSourceNames().size(), is(0));
        assertThat(innerProject.getArtifactSources().size(), is(0));

        final JavaArtifactSource javaBundle = (JavaArtifactSource)project.getArtifactSourceByName("jb-1,1,1");
        ...
        final ArtifactSource bundle = project.getArtifactSourceByName("b-1,1,1");
        ...
        final ArtifactMapping artifactMapping = bundle.getArtifactMappingByName("rm-1,1,1,1");
        assertThat(artifactMapping, is(notNullValue()));
        assertThat(artifactMapping.getExcludeds().iterator().next().getName(), is("**/*.excluded"));
    ...
    public Configuration createSampleData() {
        ...
        for (final int i : numbers) {
            final Repository repository = new Repository(configuration, "r" + i);
            repository.setActive(true);
            for (final int j : numbers) {
                final Group project = new Group(repository, "p-" + i + "," + j);
                project.setActive(true);
                final Group innerProject = new Group(project, "ip-" + i + "," + j);
                innerProject.setActive(true);
                for (final int k : numbers) {

                    final JavaArtifactSource javaBundle = new JavaArtifactSource(project, "jb-" + i + "," + j + "," + k);
                    javaBundle.setActive(true);
                    javaBundle.setInitialLookup("initialLookup");
                    javaBundle.setVirtualMachineVersion("1.5");

                    final ArtifactSource bundle = new ArtifactSource(project, "b-" + i + "," + j + "," + k);
                    bundle.setActive(true);
                    bundle.setInitialLookup("initialLookup");
                    for (final int l : numbers) {
                        final ArtifactMapping artifactMapping = new ArtifactMapping(bundle, "rm-" + i + "," + j + "," + k + ","
                                                                                            + l);
